...
for cur_url in all_urls.items():
    ...
    #print(cur_url[1])
    try:
        with open("past_pusher_failures.txt", "r") as check_file:
            #print ("file opened")
            pass
    except FileNotFoundError as fnf:
            #print ("File not found, first run, just adding things without alerting")
            first_run = 1
            with open("past_pusher_failures.txt", "w") as create_file:
                create_file.write("{}")
    with urllib.request.urlopen("{}/api/json/".format(cur_url[1])) as jsonurl:
        ...
        with open("past_pusher_failures.txt", "r") as read_file:
        ...
        with open("past_pusher_failures.txt", "w") as write_file:
